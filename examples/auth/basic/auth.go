...
// 	log.Println("\n📡 STEP 1: Sending initial request to /.well-known/auth endpoint")
// 	responseData, err := callInitialRequest(clientWallet)
// 	if err != nil {
// 		log.Fatalf("Failed to call initial request: %v", err)
// 	}
// 	log.Println("✓ Initial auth completed")

// 	// Check if server requested certificates
// 	if responseData.RequestedCertificates.Certifiers != nil && len(responseData.RequestedCertificates.Certifiers) > 0 {
// 		log.Println("\n📋 STEP 2: Server requested certificates, sending certificate response")
// 		if err := sendCertificateResponse(context.Background(), clientWallet, responseData); err != nil {
// 			log.Fatalf("Failed to send certificate: %v", err)
// 		}
// 		log.Println("✓ Certificate sent and validated")
// 	} else {
// 		log.Println("\n📋 STEP 2: No certificates requested by server")
// 	}
...
// 	}
// 	log.Println("✓ General request completed successfully")
// }
...
// // sendCertificateResponse creates and sends a certificate response to the server
// func sendCertificateResponse(ctx context.Context, clientWallet wallet.Interface, authResponse *auth.AuthMessage) error {
// 	certificateResponse, err := createCertificateResponse(ctx, clientWallet, authResponse)
// 	if err != nil {
// 		log.Printf("Failed to create certificate response: %v", err)
// 		return err
// 	}

// 	resp, err := sendCertificateRequest(ctx, clientWallet, authResponse, certificateResponse)
// 	if err != nil {
...
// 	if resp.IsError() {
// 		log.Printf("Certificate response failed with status %d: %s", resp.StatusCode(), resp.String())
// 		return fmt.Errorf("certificate response failed: %s", resp.String())
// 	}

// 	log.Printf("Certificate response successful: %s", resp.String())
// 	return nil
// }
...
// 		return fmt.Errorf("ping endpoint returned status %d: %s", resp.StatusCode(), resp.String())
// 	}
